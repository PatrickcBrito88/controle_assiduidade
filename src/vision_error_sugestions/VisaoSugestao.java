 ...
 * @author patri
 */
public class VisaoSugestao extends javax.swing.JFrame {
    
    /**
     * Creates new form VisaoErro
     */
    private static int mat;
    
    public VisaoSugestao(int mat) {
        initComponents();
        setMat(mat);
        setCampos();
    }
    
    ArrayList<SugestaoModelTabela> listaDeSugestao = new ArrayList();
    ...
    private void btnnewuser6MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnnewuser6MouseClicked
        String texto="";
        Sugestao sugestao = new Sugestao(null, true, texto, mat);
        sugestao.setVisible(true);
        String descricao=sugestao.getMotivo();
         int nCarac = sugestao.getN();
        
         String descricaoErro="";
        if (nCarac<=500){
        descricaoErro=sugestao.getMotivo();
        } else {
    ...
    public void preencheTabelaResumo() {
        ...
        TabelaSugestaoControl resumo = new TabelaSugestaoControl();
        ArrayList<SugestaoModelTabela>lista=new ArrayList();
        
        lista=resumo.ListaSugestao(mat);
        listaDeSugestao=lista;
        
        
        if (lista != null) {
            
            TabelaSugestoes tabelaDeSugestao = new TabelaSugestoes(lista);
            
            
            tableSugestao.setDefaultRenderer(Object.class, new ImagemTabela());
            tableSugestao.setRowHeight(35);
            ((DefaultTableCellRenderer) tableSugestao.getTableHeader().getDefaultRenderer()).setHorizontalAlignment(SwingConstants.CENTER);
            ((DefaultTableCellRenderer) tableSugestao.getDefaultRenderer(getClass())).setHorizontalAlignment(SwingConstants.CENTER);
            tableSugestao.getTableHeader().setFont(new Font("Segoe UI", Font.BOLD, 13));
            tableSugestao.getTableHeader().setOpaque(false);
            tableSugestao.getTableHeader().setBackground(new Color(0, 0, 0));
            tableSugestao.getTableHeader().setForeground(new Color(255, 255, 255));
            ImagemTabela img = new ImagemTabela();
            tableSugestao.setModel(tabelaDeSugestao);
        } else {
            JOptionPane.showMessageDialog(null, "Você ainda não enviou nenhuma sugestão!!");
        }
    ...
    private void btnnewuser4MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnnewuser4MouseClicked
         int linha = (tableSugestao.getSelectedRow());
           int id=(listaDeSugestao.get(linha).getId());
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(VisaoSugestao.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new VisaoSugestao(mat).setVisible(true);
            }
        });
    }
